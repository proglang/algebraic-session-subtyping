...
\begin{align*}
  ...
  \\
  & \TIn TS \grmor \TOut TS \grmor \TEnd \grmor \TDual S \grmor
  && \text{session types}
  % \\
  % & \typec{\alpha} \grmor
  % & \text{functional and session types}
  \\
  & \TPAppSet \proto J {\overline{T}} \grmor \pneg T
  && \text{protocol types}
\end{align*}
...
The session type operators are receiving, sending, termination, and the dual
operator swaps the direction of all communications in the spine of a
session type. Thus, a session type is a sequence of communications terminated
by $\TEnd$, or extensible with a type variable $\typec\alpha$ of kind $\kinds$.


If protocol $\proto$ has constructors indexed by $I$, then
$\TPAppSet \proto J {\overline{T}}$ denotes the constructor subtype restricted
to a nonempty set $J\subseteq I$.  We write
$\TPApp \proto {\overline{T}}$ for the full type
$\TPAppSet \proto I {\overline{T}}$.  For fixed parameters, constructor-set
inclusion induces subtyping:
$\TPAppSet \proto J {\overline{T}}\subt
\TPAppSet \proto K {\overline{T}}$ whenever $J\subseteq K$; the
protocol-specific rules are given in \cref{fig:alg-subtyping}.
The
...
%
\algst's session type $\TEnd$ marks a completed session
(\rulenametypeEnd), and is self-dual. The dual operator is governed by
\rulenametypeDual. Conventional session types require types for
...
order. We assume that selector tags are globally unique. Programs may use any
nonempty constructor subtype $\TPAppSet \proto J {\overline U}$ as a protocol
type. As above, omission of $J$ denotes the full constructor set $I$.
%
...
% \input{fig-polarized-constructors}
...
and \rulenametconvDualEnd from left to
...
and \rulenametconvDualEnd. The pending dualization gets reified on
a type variable.
