...
\begin{lemma}\
  ...
  \begin{enumerate}
       ...
       $\typec U \subt \typec T$ is also in normal form.
  \end{enumerate}         
  \end{proof}
\end{lemma}
...
\begin{proof}[Proof of \cref{thm:preservation} (Preservation)]\
  ...
  \begin{enumerate}
    ...
    $\expSynth{\Gamma_1,\Gamma_3}{e}{\typec T}{\Gamma_3}$, which is what we want to prove, since $\typec{T} \subt \typec{T}$.
    ...
    applying \rulenameexpApp and using the fact that $\typec{T} \subt \typec{T}$.
    ...
    recalling that $\typec{\TPair{U}{S}} \subt \typec{T}$.

    Rule Act-Send. In this case,
    $\trans{\appe{\tappe{\tappe \sendk U}{S}}{\paire v x}}{\sendl xv}{x}$.
    ...
    applying inversion of \rulenameexpApp, followed by inversion of
    \rulenameexpPair on its argument, \rulenameexpTApp twice, and then
    ...
    and knowing that $\typec{T} \subt \typec{T}$.

    Rule Act-Match. In this case, $\trans{\matchwithe xCyeiJ}{\receivel
    x{C_k}}{e_k\esubs x{y_k}}$, where $k\in K\subseteq J$. By L-Match, we have
    $\Gamma_2 = \ebind{x}{\TIn{(\TPAppSet P K {\overline{U}})}{S'}}$
    for $\protocolsfull$. Premises to \rulenameexpMatch include
    (a) $\expSynth {\Gamma_1,\ebind{x}{\TIn{(\TPAppSet P K {\overline{U}})}{S'}}, \Gamma_3} x
    {\TIn{(\TPAppSet P K {\overline{U}})}{S'}} {\Gamma_1,\Gamma_3}$,
    (b) $\expSynth {\Gamma_1,\Gamma_3,\ebind{y_i}{\matchin}} {e_i} {V_i} {\Gamma_i}$
    for every $i\in J$, and
    (c) $\typec{V} = \bigsqcup_{i \in J} \typec{V_i}$.
    ...
    Conclude applying the substitution lemma (\cref*{lem:substitution}) to (b) and (d), and using \rulenameexpCheck together with the definition of the least upper bound.

    Rule Act-Sel. We have $\trans{\appe{\tappe{\selecte{C_k}}{\overline U}}{x}}{\sendl x{C_k}}{x}$.
    Using L-Sel, we know that
    $\Gamma_2 = \ebind{x}{\TOut{(\TPAppSet P {\{k\}} {\overline{U}})}{S'}}$
    for $\protocolsfull$.
    ...
    Rule Act-Close. In this case, $\Gamma_2 = \ebind{x}{\TEnd}$ and $\Gamma_2' = \Empty$.
    Inversion of \rulenameexpApp on the second hypothesis yields 
    $\expSynth{\Gamma_1,\ebind{x}{\TEnd},\Gamma_3}{\closek}{\TFun[] UT}{\Gamma'}$
    and $\expAgainst{\Gamma'}xU{\Gamma_3}$. Apply \rulenameexpConst to the former 
    and \rulenameexpVar to the latter to conclude that $\Gamma_1=\Empty$, $\typec{U} = \TEnd$
    ...
    $\expSynth{\Gamma_1,\Gamma_2,\Gamma_3}{e_1}{U}{\Gamma_3}$ and $\typec U \subt \typec T$.
    By induction hypothesis on~\cref*{it:pres-synth} we know that 
    $\expSynth{\Gamma_1,\Gamma_2',\Gamma_3}{e_2}{U'}{\Gamma_3}$ with 
    $\typec{U'} \subt \typec{U}$. By transitivity of subtyping
    we know that $\typec{U'} \subt \typec{T}$. Using \rulenameexpCheck
...
%    $\isProc[\Gamma_1, \ebind xT , \ebind y{\TDual{T}}]{p_1}$.
    ...
    By L-Par and two applications of L-Close, we know that
    $\Normal[+]{T} = \TEnd$.
    Applying induction hypothesis on~\cref*{it:pres-proc} we get  
    $\isProc[\Gamma_1]{p_2}$.
...
\begin{lemma}[Canonical forms]
  ...
  % \item If $\typec T = \TEnd$, then $\termc v = \termc x$.
  \end{enumerate}
\end{lemma}
%
...
\begin{proof}[Proof of \cref{thm:progress} (Progress)]
  ...
  %
  When $\termc{e_1} = \closek$, we further distinguish two cases. When
  $\termc{e_2}$ is a value, canonical forms give $\termc{e_2} = \termc{x}$,
  hence $\termc{e}$ reduces by rule Act-Close. Otherwise, monotonicity
  ...
  %
  When $\termc{e_1}=\tappe{\tappe{\sendk}{W}}{X}$ and $\termc{e_2}$ is
  a value, canonical forms give $\termc{e_2}=\paire u x$, so
  $\termc e$ reduces by Act-Send. If $\termc{e_2}$ is not a value, the result
  follows by induction and Act-AppR. The subcase for
  $\tappe{\selecte C}{\overline W}$ is similar.
  ...
  Rule \rulenameexpMatch. In this case $\termc{e} = \matchwithe{e'}CxeiJ$. The
  premises to the rule include $\expSynth{\Gamma_1^{\kinds}}{e'}{S}{\Gamma_2}$ and
  $\Normal[+]{S}={\TIn{(\TPAppSet P K {\overline U})}{S'}}$, where
  $\varnothing\ne K\subseteq J$. Agreement
...
\begin{center}
  \begin{tabular}{cc}
    ...
    $\parl{\closel x}{\closel y}$ & Act-Wait\\
