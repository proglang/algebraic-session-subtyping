...
This section introduces the notions of expressions and process configurations,
algorithmic typing for expressions, and operational semantics for expressions
and configurations based on labelled transition relations.
%
% \paragraph*{Syntax}
%
The syntax of constants $\termc c$, values $\termc v$, expressions
$\termc e$, and process configurations $\termc K$ is defined by the
grammar below.
%
\begin{align*}
  \termc c \grmeq & \unitk
...
receive a value on a given channel, $\sendk$ to send a payload--endpoint pair,
$\closek$ to close a completed channel endpoint, and $\appe \selectk {C}$ to
exercise a choice on a given
...
type language, hence we have made it an expression ($\matchwithe eCxeiJ$).
%
At the level of processes, a configuration $\termc{K=(E,L)}$ consists of a
finite multiset $E$ of expressions and a finite set $L$ of live channels.  Each
element
$\{x,y\}\in L$ is an unordered pair of distinct variables naming the two ends
of one channel.  The expressions are closed with respect to type variables and
may share the channel-end variables recorded by $L$.  The multiset $E$ is the
pool of parallel threads; multiplicity distinguishes separate occurrences of
the same expression, while order is immaterial.  Closing a channel removes its
pair from $L$, while allocation adds a pair of fresh variables.  This flat
representation avoids structural congruence and scope-extrusion rules.
Configuration typing additionally requires the two endpoints in every pair to
have dual session types.

% \paragraph{Typing}

%\input{fig-ctx}
\input{fig-type-constants}
\input{fig-alg-subtyping}
...
%
Algorithmic subtyping is invoked only by rule \rulenameexpCheck, and both types
in its premise are in normal form.  \Cref{fig:alg-subtyping} gives the two
protocol-specific clauses, where $I$ and $J$ are nonempty subsets of the
constructors of $\proto$.  The relation
$\paramsSubt{\proto}{P}{Q}$ compares parameters pointwise according to the
variance inferred from the declaration of $\proto$.  Writing
$\mathsf{var}_{\proto}(i)$ for the variance of its $i$th parameter, we define
\[
  \paramsSubt{\proto}{P}{Q}
  \quad\Longleftrightarrow\quad
  \text{for every }i,\quad
  \begin{cases}
    \typec{P_i}\subt\typec{Q_i}
      & \text{if }\mathsf{var}_{\proto}(i)=+,\\
    \typec{Q_i}\subt\typec{P_i}
      & \text{if }\mathsf{var}_{\proto}(i)=-,\\
    \typec{P_i}=\typec{Q_i}
      & \text{if }\mathsf{var}_{\proto}(i)=0.
  \end{cases}
\]
Thus invariant parameters are equal as normal forms.  Input is covariant in
its protocol argument, whereas output is contravariant; both are covariant in
their continuation.  Protocol inversion reverses subtyping:
$\typec P\subt\typec Q$ if and only if
$\pneg Q\subt\pneg P$.  The remaining clauses are the standard
syntax-directed structural rules and are omitted.  The mechanization contains
the full inductive definition and its decision procedure.
%
...
Rule \rulenameexpMatch is parametric in the implicit set of protocol
declarations. Suppose that protocol $\proto$ has constructors indexed by $I$.
The branches range over a nonempty set $J\subseteq I$, while synthesis for the
scrutinee may return
$\TIn{(\TPAppSet \proto K {\overline U})}{S}$ for any nonempty
$K\subseteq J$. Thus the match must cover every constructor that the scrutinee
may select, but may contain additional branches. For each $i\in J$, the
corresponding branch is then typed in turn. The
...
$\typec{V_i}$ and context $\Gamma_i$, for $i\in J$. In all branches,
...
Rule \rulenameexpCheck relies on the previous judgment to check expression $\termc e$ against
...
The operational semantics for expressions and process configurations is
defined via transition relations labelled by the actions below.
%
\begin{align*}
...
\end{align*}
%
The $\sigma$ labels capture value and selector communication and the close
action on a completed session endpoint.
%
Labels $\lambda$ are for expressions: $\beta$ is for internal actions,
$\forkl v$ records a forked value, and $\newl xyT$ records the type and the two
fresh endpoint names of an allocated channel.
%
Process labels retain only the information visible at the configuration
level: $\tau$ is an internal step and $\newl xyT$ announces allocation of the
fresh channel $\{x,y\}$.  Session actions do not escape as process labels; they
either synchronize two distinct threads or leave the thread waiting for a
partner.

\input{fig-label-exps-abbrev}
...
lambda calculus.  Rules Act-Fork and Act-New record, respectively, the value
forked and the type and endpoint names of the freshly allocated channel.
These transitions are then handled at the configuration level.  The remaining
observable labels cover value communication, selector communication, and
closing.
...
\input{fig-label-procs}

The labelled transition system for configurations is defined by the rules in
\cref{fig:lts-procs}.  Multiset union is written $\uplus$.  A source of the
form $(\mset{e}\uplus E,L)$ distinguishes one occurrence of $e$ and leaves the
remainder $E$ unchanged; similarly, $(\mset{e_1,e_2}\uplus E,L)$ distinguishes
two thread occurrences.  Thus the rules require neither thread positions nor
an explicit premise that the two occurrences are distinct.  The notation
$x\notin K$ means that $x$ occurs neither in an expression of $K$ nor in a pair
in its set of live channels.

Rules Act-Beta and Act-Fork lift independent expression transitions for an
arbitrary thread; the latter adds the application $\appe v\unitk$ as a
new thread.  Rule Act-New chooses distinct names $x$ and $y$ that are fresh for
the entire source configuration, exposes them in the allocation label, and
adds $\{x,y\}$ to the set of live channels.  The remaining rules synchronize
two thread occurrences whose endpoints form a pair in $L$.  They replace both
occurrences in the multiset, and Act-Wait additionally removes the synchronized
pair from $L$.  Their synchronization discipline follows prior labelled
semantics for linear functional languages with sessions
\cite{DBLP:conf/concur/0001KDLM21,DBLP:journals/corr/abs-2105-08996,DBLP:journals/corr/abs-2106-11818}.
In the flat presentation, parallel composition, restriction, scope opening,
and their associated structural rules are absorbed by multiset decomposition
and the explicit set of live channels.  The mechanization gives the full formal
details of a concrete representation and its permutation invariance.
