...
recursive session type $\mu\alpha.S \approx S[\mu\alpha.S /
\alpha]$. Our translation to AST proceeds as follows.
First, we translate \emph{all} types in a TST program in a system of
...
to type variables. Here are the equation systems for our running
examples.
\begin{lstlisting}
...
labeled transition system (LTS), so in a second step, we minimze this
automaton. This minimization corresponds to equating bisimilar type
...
iff they are represented by the \emph{same} type
...
The final step of the translation reads off algebraic protocols from
...
types. For \lstinline|IntListS| and \lstinline|IntStreamS|, we obtain the following algebraic protocols.
\begin{lstlisting}
  protocol ConsNil a b = Cons a | Nil b
...
\lstinline|Repeat| are tagfree as they do not transmit a tag.
...
restrictable variants \cite{DBLP:conf/ecoop/MadsenSL23}, where a
...
\lstinline{!IntListP} interprets the protocol in the send direction
(corresponding exactly to \lstinline{IntListS}), whereas
\lstinline{?IntListP} interprets the protocol in the receive direction
(corresponding exactly to the dual of \lstinline{IntListS}). Taking
directionality into account, we obtain the following non-reflexive subtypings:
\begin{lstlisting}
!IntListP <: !IntListP[Nil]                ?IntListP[Nil]  <: ?IntListP
!IntListP <: !IntListP[Cons]               ?IntListP[Cons] <: ?IntListP
\end{lstlisting}
The resulting subtyping relation on restrictable algebraic protocols
...
\cite{DBLP:journals/toplas/Padovani19}. That is, restrictable
algebraic protocols carve out a subset of context-free session types
with an efficiently decidable subtyping relation.

This subtyping relation is key in defining a mapping from traditional,
tail-recursive session types to algebraic protocols. To construct this
mapping, we first define a normal form for recursive types such that
recursive $\mu$ binders only occur directly on top of selection
$\oplus$ or branch $\&$ types, if the type contains finite
sequences. From this normal form, we construct a single
unified algebraic protocol, from which we define subtypes by
restriction that correspond to the traditional types.


To cover subtyping in the traditional setting, we represent all types
in a program by a system $\mathcal{E}$ of equations between flat types. A flat type
consists of a type constructor applied to type variables as in
\lstinline{alpha->beta} or \lstinline{?alpha.beta}. 
For example, here are the equation systems for our running examples.

XXX

A type is then a pair $\langle\alpha, \mathcal{E}\rangle$ where $\alpha$ is the left
hand side of an equation in $\mathcal{E}$. Subsumption keeps the
variable $\alpha$; it only changes $\mathcal{E}$ where the standard
subsumption may be applied to any suitable equation. For instance,
here is a subsumption  of \lstinline{IntListS} where we eliminate the constructor \lstinline{Cons}:
\begin{lstlisting}
IntListS[Nil]    alpha = (+){Nil: delta}   beta = !gamma.alpha    gamma = Int   delta = End
\end{lstlisting}
Importantly, we keep the domain of $\mathcal{E}$, i.e., all left hand
sides stay the same. 
