...
\end{lstlisting}
In session types, the $\oplus$ constructor stands for selection: send a label like
\lstinline|Cons| or \lstinline|Nil| and continue according to the
chosen branch; the $!T.S$ constructor stands for sending a value of
...
decidable in $O(n^2)$ for traditional, end-recursive session types
...
exponential\cite{DBLP:conf/tacas/AlmeidaMV20}, though recent work indicates it might be polynomial
...
end-recursive or context free.

The prior work also gave an informal outline of an embedding of
algebraic protocols into context-free session types, but did not offer
a comparison with traditional, end-recursive session types. The
...
subtyping for end-recursive session types is decidable in $O(n^2)$ and
