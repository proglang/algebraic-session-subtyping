...
Session types \cite{DBLP:conf/concur/Honda93,DBLP:conf/parle/TakeuchiHK94,DBLP:conf/esop/HondaVK98} have become a quasi-standard for defining and
reasoning about stateful communication protocols. A (binary) session
...
close) and \lstinline{IntStreamS} (send infinitely many integers):
\begin{lstlisting}
  type IntListS   = mualpha. oplus{Cons: !Int.alpha, Nil: EndT} -- finite sequences
  type IntStreamS = mualpha. !Int.alpha                    -- infinite sequence
\end{lstlisting}

Using recursive types directly is somewhat difficult as equivalence of
...
tractable for traditional, end-recursive session types ($O(n^2)$ where $n$ corresponds to the size of the type),
...
\cite{DBLP:conf/icfp/ThiemannV16,DBLP:conf/esop/Padovani17,DBLP:journals/toplas/Padovani19,DBLP:journals/corr/abs-2106-06658,
  DBLP:conf/esop/DasDMP21} where the first equivalence test was double
exponential, though recent work has shown it to be polynomial
...
We define a subtyping relation of parameterized algebraic
...
subtype of an algebraic data type is defined by a non-empty subset of the
...
and \lstinline{IntListP[Nil,Cons]} which roughly correspond to the
session type \lstinline|oplus{Nil: EndT}|,
...
(corresponding exactly to \lstinline{dual IntListS}). Taking
...
is also decidable in linear time in the size of the type under the
assumption that comparing two sets of constructors takes constant
...
algebraic protocols constitute a subset of context-free session types
