...
| `ExprNormalTyping` | Linear/unrestricted/used bindings, threaded contexts, value typing, synthesis, checking, branch joins, and primitive types. | Central typing definition; depends on normal-form algorithmic subtyping and merge. |
...
- Prove the local expression canonical-forms/progress result needed to
  ...
  premises.
- Make fresh configuration channel kinds consistent with `close` and
  `select`: `LiveCtx` currently stores live endpoints at `SLin`, whereas the
  primitive operations still expect `TLin`-wrapped inputs.
